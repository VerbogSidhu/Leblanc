...
final class IGDBClient {
    ...
    func metadata(for steamAppID: String?) async -> GameMetadata? {
        guard let token = await ensureToken(), let appID = steamAppID else { return nil }
        return await queryIGDB(token: token, whereClause: "external_games.uid = \"steam_\(appID)\"")
    }

    func metadata(forTitle title: String) async -> GameMetadata? {
        guard let token = await ensureToken() else { return nil }
        let escaped = title.replacingOccurrences(of: "\"", with: "\\\"")
        return await queryIGDB(token: token, whereClause: "name ~ *\"\(escaped)\"*")
    }
